...
def save_checkpoint(
    ...
    config_values = dict(config.to_dict())
    config_values.pop("device", None)
    state = {
        "round": round_idx,
        "config": config_values,
        "global_model_state": global_model.state_dict(),
        "denoiser_state": denoiser.state_dict(),
        "diffusion_trainer_state": diffusion_trainer.state_dict(),
        "ema_prototypes": {
        ...
        else synthetic_features.detach().cpu().clone(),
        "synthetic_labels": None
        if synthetic_labels is None
        else synthetic_labels.detach().cpu().clone(),
    }
...
def evaluate_checkpoint(
    ...
    _, test_loader = get_cifar100_federated(config)
    ...
    standard_accuracy = evaluate_global_model(
        model=global_model,
        test_loader=test_loader,
    ...
    metrics = {"test_accuracy": standard_accuracy}
